...
import 'package:table_tennis_statistics/UI/components/InputMatchResult.dart';
import 'package:table_tennis_statistics/UI/components/InputSetResult.dart';
...
class _AddMatchState extends State<AddMatch> {
  ...
  List<TextEditingController> _textEditingController =
      new List<TextEditingController>.generate(
          16, (index) => new TextEditingController());
  ...
  void resetTextControllers(int v) {
    for (int i = v; i < _textEditingController.length; i++) {
      if (_textEditingController[i] == null)
        continue;
      else
  ...
  List createSetResultArray() {
    var setResult = [];
    for (int i = 0; i < _voySet + _dmnSet; i++) {
      Map set = {};
      set = {
        'voy': _voySetPoints[i],
        'dmn': _dmnSetPoints[i],
  ...
  Widget build(BuildContext context) {
    var matchProvider = Provider.of<CRUDModel>(context);
                                ...
                                children: [
                                  ...
                                  if (_voySet + _dmnSet > 6)
                                    InputSetResult(
                                      ...
                                      voyController: _textEditingController[14],
                                      dmnController: _textEditingController[15],
                                      voyOnSubmitted: (text) {
                                        FocusScope.of(context)
                            ...
                            child: FlatButton(
                              ...
                              onPressed: () async {
                                await matchProvider.addMatch(
                                  Match(
                                    date: _date,
                                    result: {"voy": _voySet, "dmn": _dmnSet},
                                    setResult: this.createSetResultArray(),
                                  ),
                                );
                                this.showAddedMatchDialog();
                                this.resetPoints();
                                this.resetSets();
                                this.resetTextControllers(0);
                              },
                              child: Column(
                                children: <Widget>[
                                  Text("Dodaj"),
